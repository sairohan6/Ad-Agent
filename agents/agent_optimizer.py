...
# SYSTEM_PROMPT_TMPL = """
# You are an expert anomaly-detection engineer.

# We are tuning the model's hyperparameters.
...
# 1) Two lines:
# Thought: <explain>
# Action: execute_code({"param_name": value, ...})
...
# class ModelParamInjector(ast.NodeTransformer):
#     """
#     Safely inject new parameters into model constructor call:
#     model = ModelName(arg1=..., arg2=..., ...)
#     """
...
#             # Replace or insert the new parameters
#             existing = {arg.arg for arg in node.keywords if arg.arg is not None}
#             for k, v in self.new_params.items():
#                 if k in existing:
#                     # Modify existing param
#                     for kw in node.keywords:
#                         if kw.arg == k:
#                             kw.value = ast.parse(repr(v)).body[0].value
#                 else:
#                     # Add new param
#                     node.keywords.append(ast.keyword(arg=k, value=ast.parse(repr(v)).body[0].value))
#         return node


# class AgentOptimizer:
#     _ACTION_RE = re.compile(r"Action:\s*execute_code\((.*)\)")
#     _FINAL_RE = re.compile(r"Final:\s*(\{.*\})")

#     @staticmethod
#     def execute_code(parameters: Dict[str, Any], base_code: str, algorithm_name: str) -> str:
#         # Parse → inject params → generate new runnable script
#         try:
#             tree = ast.parse(base_code)
#             injector = ModelParamInjector(parameters)
#             new_tree = injector.visit(tree)
#             new_code = ast.unparse(new_tree)
#         except Exception:
#             return "[ERROR] AST injection failed."

#         folder = "./generated_scripts"
#         os.makedirs(folder, exist_ok=True)
#         script_path = os.path.join(folder, f"{algorithm_name}.py")

#         with open(script_path, "w", encoding="utf-8") as f:
#             f.write(new_code)

#         try:
#             proc = subprocess.run(["python", script_path], capture_output=True, text=True, timeout=70)
#             return (proc.stdout + proc.stderr).strip()
#         except subprocess.TimeoutExpired:
#             return "[ERROR] Execution timeout."

#     def _extract_params(self, content: str) -> Optional[Dict[str, Any]]:
...
#     @staticmethod
#     def _extract_final(content: str):
#         import json
#         m = AgentOptimizer._FINAL_RE.search(content)
#         if not m:
#             return None
#         try:
...
#     @staticmethod
#     def _find_metric(label, text, default=-1.0):
#         m = re.search(fr"{label}:\s*([0-9.]+)", text)
#         return float(m.group(1)) if m else default

#     @staticmethod
#     def _parse_error_points(text):
#         pts = []
#         for line in text.splitlines():
#             if "Failed prediction at point" in line:
#                 m = re.search(r"\[([^\]]+)] with true label ([\d.]+)", line)
#                 if m:
#                     pts.append({"point": [float(x) for x in m.group(1).split(",")], "true_label": float(m.group(2))})
#         return pts

#     def run(self, quality: CodeQuality, algorithm_doc: str, max_steps=8):
#         code, params, output, name = quality.code, quality.parameters, quality.std_output, quality.algorithm

#         for step in range(max_steps):
#             prompt = SYSTEM_PROMPT_TMPL.format(code=code, parameter=params, std_output=output, algorithm_doc=algorithm_doc)
#             response = query_gemini(prompt)

#             final = self._extract_final(response)
#             if final:
#                 params = final.get("best_params", params)
#                 break

#             new_params = self._extract_params(response)
#             if not new_params:
#                 continue

#             params = new_params
#             output = self.execute_code(params, code, name)

#         final_output = self.execute_code(params, code, name)
#         return CodeQuality(
#             code=code,
#             algorithm=name,
#             parameters=params,
#             std_output=final_output,
#             error_message=quality.error_message,
#             auroc=self._find_metric("AUROC", final_output, quality.auroc),
#             auprc=self._find_metric("AUPRC", final_output, quality.auprc),
#             error_points=self._parse_error_points(final_output),
#             review_count=quality.review_count
#         )
